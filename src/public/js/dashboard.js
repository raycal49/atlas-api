...
const periodLabel = (start, end) => {
  ...
  const year = new Date(end).toLocaleDateString(undefined, {
    timeZone: 'UTC',
    year: 'numeric',
  });
...
const renderKpis = (data, apis) => {
  ...
  document.querySelector('#planPrice').textContent = planPriceLabel(
    data.price,
    data.pending_plan,
    data.bill_due,
  );
  ...
  document.querySelector('#nextBillIn').textContent = daysLabel(
    daysUntil(data.bill_due),
  );
...
const renderUsage = (apis) => {
  ...
  for (const api of apis) {
    ...
    counts.textContent =
      `${formatCount(api.calls_used)} / ${formatCount(api.monthly_limit)}` +
...
async function loadDashboard() {
  try {
    ...
    document.querySelector('#planSince').textContent = new Date(
      dashboard.plan_start,
    ).toLocaleDateString();

    document.querySelector('#usagePeriod').textContent = periodLabel(
      dashboard.bill_start,
      dashboard.bill_due,
    );
